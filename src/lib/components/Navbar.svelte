...
>
    <div class="container mx-auto px-4 flex justify-between items-center">
            ...
            {#each navLinks as link}
                ...
                >
                    ...
                    {#if page.url.pathname === link.href}
                        <span
                            class="absolute inset-0 rounded-full ring-1 ring-white/20"
                        ></span>
                    {/if}
                </a>
            {/each}
        ...
        <div class="hidden md:flex items-center gap-4">
            ...
            <Button
                href="/register"
                class="bg-primary hover:bg-primary/90 text-white rounded-full px-6 shadow-[0_0_15px_rgba(139,92,246,0.5)] hover:shadow-[0_0_25px_rgba(139,92,246,0.7)] transition-all duration-300"
            >
                Join Club
            </Button>
        </div>
    ...
    >
        ...
        <div class="flex flex-col gap-4 w-full max-w-xs px-8">
            <Button
                href="/login"
                variant="outline"
                class="w-full justify-center py-6 text-lg border-white/20 hover:bg-white/10"
                onclick={closeMobileMenu}
            >
                Login
            </Button>
            <Button
                href="/register"
                class="w-full justify-center py-6 text-lg bg-primary hover:bg-primary/90 shadow-[0_0_20px_rgba(139,92,246,0.4)]"
                onclick={closeMobileMenu}
            >
                Join Club
            </Button>
